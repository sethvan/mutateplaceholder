...
#  define DEBUG_GDB_BREAK_POINT() do{}while(0)
#elif defined(X86)
...
enum class ParseArgvStatusCode : unsigned char {
	SUCCESS,
	ERROR,
	SHOWHELP,
	SHOWVERSION
...
// remove special characters from a string so it can be safely shown in the console without risk of introducing security vulnerabilities
std::string sanitizeOutputMessage(const char * input);

std::string sanitizeOutputMessage(std::string input);

// Check for unicode white spaces as well as ascii , if not white space returns 0, if white space returns the amount of bytes it takes up
// Parameter end is .end() of std::string
