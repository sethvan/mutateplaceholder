...
ParseArgvStatusCode parseArgs(CLIOptions* output, std::vector<std::string>* nonPositionals, int argc,
    ...
    while (nextStartIndex < argc) {
        ...
        // The following logic below ensures that non-positional arguments can be mixed with options even in libraries
        // with a POSIX compliant getopts_long
        //  The issue is that strict POSIX requires the first non-option argument and all successive arguments to be
        //  treated as non-positions, which is no fun.
