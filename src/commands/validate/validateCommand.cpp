...
std::string printValidateHelp(std::string indent) {
	return printValidateHelp( indent.c_str() );
}

std::string printValidateHelp(void) {
	return printValidateHelp("");
}
...
void validateValidateArgs(CLIOptions * opts, std::vector<std::string> * nonpositionals) {
	...
	if (opts->hasPenetration()) throw InvalidArgumentException("Cannot use the --penetration option in validate mode");
	if (opts->hasFormat()) throw InvalidArgumentException("Cannot use the --format option in validate mode");
	if (1 < nonpositionals->size()) throw InvalidArgumentException("validate mode does not accept extra non-positional arguments");
...
void doValidateAction(CLIOptions * opts, std::vector<std::string> * nonpositionals) {
	...
	opts->putResOutput( "100% of mutations match source code lines...0/0 mutations do match or something...Just make this text look pretty and functional and colored when implementing this" );
