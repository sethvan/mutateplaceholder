...
Mutator::Mutator(std::string _sourceString, std::string& _outputString, SelectedMutVec _selectedMutations, CLIOptions* _opts)
...
void Mutator::multilineReplace(std::string& subject, const SelectedMutation& sm) {
	...
	bool addIndentation = false, doesNotMach = false;
	...
	while((pos = subject.find(lines[0], pos)) != std::string::npos) {
		...
		if((substringIsMatch(subject, startPos, std::string(sm.pattern)))) {
			if((lastNonWhiteSpace(begin, end)) != std::string::npos) {  ++pos;	continue; }
			++matches;
			subject.replace(pos, sm.pattern.length(), sm.mutation);
			pos += sm.mutation.length();
			continue;
		...
		if(!(lineEdgesAreGood(begin, end, *linesIt))) {  ++pos;	continue; }
		
		//At this point lines[0] has matched its line		
		if(!(substringIsMatch(subject, end, *(++linesIt)))) {
			if(indentation) addIndentation = true; // try again but now indented
			else {	++pos;	continue; }
			if(!(wholeLineDoesMatch(*linesIt))) {  ++pos;	continue; }
		}
		else if(!(lineEdgesAreGood(begin, end, *linesIt))) {  ++pos;  continue; }
		...
		std::string indentedPermutation;
		if((isMultilineString(sm.mutation)) && addIndentation) {
			std::vector<std::string> permLines = separateLinesIntoVector(sm.mutation);	
			indentedPermutation = permLines[0];		
			for(size_t i{}; i < permLines.size(); ++i) {
				if (i != 0) 
					indentedPermutation += indent + permLines[i];
			}			 
		}
		++matches;		
		subject.replace(pos, end - startPos, (addIndentation? indentedPermutation : sm.mutation));
		pos += addIndentation? indentedPermutation.length() : sm.mutation.length();
