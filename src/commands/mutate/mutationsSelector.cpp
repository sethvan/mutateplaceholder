...
/*
 * mutationsSelector.cpp: This class randomly selects from the possible mutations captured by the MutationsRetriever class. 
 *
...
#define printDatos() std::cout << "lineNumber: " << it->data.lineNumber << ", depth: " << it->data.depth \
...
#define OFFSET ((it->data.depth ? it->data.depth -1 : 0) + it->data.isOptional + it->data.isNewLined + it->data.mustPass)
...
void MutationsSelector::selectMutations() {
	...
	std::sort(selectedMutations.begin(), selectedMutations.end(), [](auto a, auto b){return a.data.lineNumber > b.data.lineNumber;});
}
...
void MutationsSelector::setSelectedMutCount() {
	if (opts->hasMutCount()) {
		selectedMutCount = opts->getMutCount();
	}
	else {
...
void MutationsSelector::selectPermutation(size_t index, PossibleMutVec::iterator& it) {
	index = index > (it->permutations.size()-1)? it->permutations.size()-1 : index; // for synced lines with less permutations than leader
	...
	std::string_view pattern(it->pattern.c_str() + offset, (endPos == std::string::npos? it->pattern.size() : endPos + 1)); 
	std::string_view mutation = it->permutations[index]; 
	selectedMutations.emplace_back(pattern, mutation, it->data);
	//printDatos();
}

void MutationsSelector::groupedSelectPermutation(const std::vector<size_t>& indexes, size_t groupNumber, PossibleMutVec::iterator& it) {
...
void MutationsSelector::addAnythingElseNested(const std::vector<size_t>& indexes, size_t groupNumber, PossibleMutVec::iterator& it) {
	auto upwardsIt = it;
	...
	while(!((it+1)->data.groupNumber) && !((it+1)->data.isOptional) && it+1 != possibleMutations.end() && (it+1)->data.depth > it->data.depth) {
		++it;
		groupedSelectPermutation(indexes, groupNumber, it); 
	}
}

void MutationsSelector::addNewGroup(std::vector<size_t>& indexes, size_t& newGroupNumber, PossibleMutVec::iterator& leader) {
...
void MutationsSelector::addNestedLine(const std::vector<size_t>& indexes, size_t groupNumber, PossibleMutVec::iterator& it) {
	groupedSelectPermutation(indexes, groupNumber, it);					
