...
void CLIOptions::setSrcOrTsvInput(FILE **srcOrTsv, const char *path, const char *mode, int bufferMode, const char *which) {
...
void CLIOptions::setSeedInputOrOutput(FILE** inOrOut, const char* path, const char* mode, int bufferMode, const char* which) {
	if (*inOrOut != nullptr) {
...
void CLIOptions::setMinOrMaxMutCount(std::optional<std::int32_t> *minOrMax, const char *count, const char *shortName, const char *fullName) {
	...
	*minOrMax = (std::int32_t)retStatus;
	...
	if (mutCount.has_value() &&	(minMutCount.has_value() ||	maxMutCount.has_value())) {
		throw InvalidArgumentException( "option groups --count and --max-count/--min-count are mutually exclusive. Please choose one group");
	}
}

void CLIOptions::setMutCount(const char *count) {
	setMinOrMaxMutCount(&(mutCount), count, "", "mutation count");
}
...
void CLIOptions::setMinMutCount(std::int32_t count) {
	minMutCount = count;
}
...
void CLIOptions::setMaxMutCount(std::int32_t count) {
	maxMutCount = count;
}

void CLIOptions::setPenetration(const char *count) {
	setMinOrMaxMutCount(&(penetration), count, "", "penetration");
}
...
void CLIOptions::putSeedOutput(std::string result) {
	writeStringToFileHandle(seedOutput, result);
}

bool CLIOptions::hasSeed() {
	return seedString.has_value() || seedInput != nullptr;
}
...
bool CLIOptions::hasFormat() { 
	return format.has_value(); 
}

bool CLIOptions::seedNeedsExporting() { 
	return seedOutput != nullptr; 
}
...
void CLIOptions::addWarning(std::string str) {
	warnings.push_back(sanitizeOutputMessage(str));
}
...
std::string CLIOptions::getWarnings() {
	...
	if(noMatchLines.size()) {
		os << "The pattern cell" << (noMatchLines.size() > 1? "s" : "") << " beginning at the" << (noMatchLines.size() > 1? "se" : "") 
		<< " following line number" << (noMatchLines.size() > 1? "s" : "") << " had no match" << (noMatchLines.size() > 1? "es" : "") << " in the source file: { ";
	...
	if(multipleMatchLines.size()) {
		os << "The pattern cell" << (multipleMatchLines.size() > 1? "s" : "") << " beginning at the" << (multipleMatchLines.size() > 1? "se" : "") 
		<< " following line number" << (multipleMatchLines.size() > 1? "s" : "") << " had multiple matches" << (multipleMatchLines.size() > 1? "es" : "") << " in the source file: { ";
...
void CLIOptions::addNoMatchLine(int n) {
	noMatchLines.push_back(n);
}
						
void CLIOptions::addMultipleMatchLine(int n) {
	multipleMatchLines.push_back(n);
}