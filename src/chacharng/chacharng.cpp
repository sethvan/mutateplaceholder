...
#define CHACHARNG_QR(a, b, c, d) (			\
	a += b,  d ^= a,  d = CHACHARNG_ROTL(d,16),	\
	c += d,  b ^= c,  b = CHACHARNG_ROTL(b,12),	\
	a += b,  d ^= a,  d = CHACHARNG_ROTL(d, 8),	\
	c += d,  b ^= c,  b = CHACHARNG_ROTL(b, 7))
...
State::State(std::uint8_t * inSeed) {
	seed( inSeed );
}
...
void State::seed(std::uint8_t * inSeed) {
	...
	for (int i=0; i < 8; i++)
		this->block[i+4] = (inSeed[i*4+0] << 24) | (inSeed[i*4+1] << 16) | (inSeed[i*4+2] << 8) | inSeed[i*4+3];
...
State::result_type State::operator()() {
	return next32();
}
