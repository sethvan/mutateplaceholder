...
using commandsMap = const std::unordered_map<std::string_view, ParseArgvStatusCode (*)(CLIOptions *, std::vector<std::string> * )>;
...
static ParseArgvStatusCode parseArgvAndPerformAction(int argc, const char **argv) {
	...
	if (status == ParseArgvStatusCode::SUCCESS && 1 < argc && 0 == nonpositionals.size()) {
		throw InvalidArgumentException("No command specified (must be one of 'mutate', 'highlight', 'score', or 'validate')\n");
	}
...
static int processFinalStatus(ParseArgvStatusCode status) {
	...
	switch (status) {
		...
		case ParseArgvStatusCode::SHOWHELP:
			...
			//           "  --version                ";
			std::cout << indent << "-i, --input=FILE         Source code file to apply mutations to. Defaults to stdin\n";
			std::cout << indent << "-m, --mutations=FILE     Mutations TSV file containing mutations. Defaults to stdin\n";
			std::cout << indent << "-o, --output=FILE        Write mutated source code to this file. Defaults to stdout\n";
