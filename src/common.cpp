...
std::string sanitizeOutputMessage(std::string input) { return sanitizeOutputMessage(input.c_str()); }
    ...
    if ((it + 1) != end) {
        ...
        if ((it + 2) != end) {
            ...
            if (first == 0xE2 && second == 0x80) {
                if ((third >= 0x80) && (third <= 0x8A || third == 0xAF || third == 0xA8 || third == 0xA9)) return 3;
            }
        }
    }
    return 0;
