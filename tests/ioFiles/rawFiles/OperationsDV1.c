...
char cardNumber[15][12] = {"first", "second", "third",    "fourth",  "fifth",      "sixth",      "seventh",  "eighth",
...
void display_output_spreadsheet(StringArray optionsArray, StringArray factorsArray, float *compatability,
                                float *pfGrade) {
    int line = 0;
    printf("\n\n\n%38s%-80s\n", " ", "          Compatabilities of the options with each corresponding factor");
    printf("%38s", "Factor     ");
    for (line = 0; line < 81; line++) putchar('-');
    ...
    if (optionsArray.size < 5) {
        for (line = 0; line < 79 - (optionsArray.size * 16); line++) putchar(' ');
        putchar('|');
    }
    printf("\n");
